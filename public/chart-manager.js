...
class ChiefChartManager {
  constructor() {
    ...
    // Chief Petroleum brand colors - Dark theme optimized
    this.colors = {
      primary: '#8B0000', // Dark red
      secondary: '#330000', // Darker red
      accent: '#ff6b35',
      gold: '#ffd700',
      success: '#10b981',
      warning: '#f59e0b',
      danger: '#ef4444',
      gray: '#a0aec0' // Lighter gray for dark theme
    };
  ...
  updateTheme(isDark) {
    ...
    Chart.defaults.color = isDark ? '#e2e8f0' : '#4a5568';
   ...
   */
  prepareSalesTrendData(dataService, period) {
    // Use filtered data if available
    const currentData = dataService.dashboard?.getFilteredData() || dataService.getCurrentData();
    const salesTrend = dataService.getSalesTrendData(currentData, period);
    
    return {
      labels: salesTrend.map(item => this.formatPeriodLabel(item.period, period)),
      sales: salesTrend.map(item => item.sales),
      profit: salesTrend.map(item => item.profit)
    };
  }
   ...
   */
  prepareProductData(dataService) {
    const currentData = dataService.getCurrentData();
    const productAnalysis = dataService.getProductTypeAnalysis(currentData);
    
    return {
      labels: productAnalysis.map(item => item.type),
      values: productAnalysis.map(item => item.sales)
    };
  }
   ...
   */
  prepareCustomerData(dataService) {
    const currentData = dataService.getCurrentData();
    const topCustomers = dataService.getTopCustomers(currentData, 8);
    
    return {
      labels: topCustomers.map(customer => {
        // Truncate long customer names
        return customer.name.length > 20 
          ? customer.name.substring(0, 17) + '...' 
          : customer.name;
      }),
      sales: topCustomers.map(customer => customer.sales),
      profit: topCustomers.map(customer => customer.profit)
    };
  }
   ...
   */
  prepareProfitData(dataService) {
    const currentData = dataService.getCurrentData();
    // Sample transactions for scatter plot
    const points = currentData.records
      .filter(record => record['Sales'] > 0 && record['Actual Profit By Item'] > 0)
      .slice(0, 100) // Limit points for performance
      .map(record => ({
        x: record['Sales'],
        y: record['Actual Profit By Item']
      }));
  ...
  prepareCumulativeData(data) {
    const salesTrend = data.getSalesTrendData ? data.getSalesTrendData(data, 'monthly') : [];
